...
function GroupsTab({ me }) {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      loadGroups();
    } catch (err) { toast.error(err.response?.data?.message ?? 'Lỗi tạo nhóm'); }
    finally { setSaving(false); }
  };
  ...
  const handleAddMember = async () => {
    ...
    try {
      await api.post(`/maintenance-groups/${detailGrp.groupId}/members`, { employeeId: Number(addEmpId) });
      toast.success('Đã thêm thành viên');
      await openDetail(detailGrp);
    } catch (err) { toast.error(err.response?.data?.message ?? 'Lỗi thêm thành viên'); }
  };
  ...
  const handleRemoveMember = async (empId) => {
    try {
      ...
      await openDetail(detailGrp);
    } catch (err) { toast.error(err.response?.data?.message ?? 'Lỗi xóa thành viên'); }
  };
    ...
    <div className="space-y-4">
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Tạo nhóm bảo trì" size="sm">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <Textarea label="Mô tả" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} placeholder="Mô tả nhiệm vụ của nhóm..." />
          <div className="flex justify-end gap-3 pt-2">
            <Button type="button" variant="secondary" onClick={() => setCreateOpen(false)}>Hủy</Button>
      ...
      {/* Modal thành viên */}
      <Modal open={!!detailGrp} onClose={() => setDetailGrp(null)} title={`Nhóm: ${detailGrp?.groupName ?? ''}`} size="md">
        {detailGrp && (
          <div className="space-y-4">
            {/* Danh sách thành viên */}
            <div>
              <h4 className="text-sm font-semibold text-gray-700 mb-2">Thành viên ({members.length})</h4>
              {members.length === 0
                ? <p className="text-sm text-gray-400 bg-gray-50 rounded-lg px-4 py-3">Chưa có thành viên</p>
                : (
                  <div className="space-y-1.5 max-h-48 overflow-y-auto">
                    {members.map(m => (
                      <div key={m.employeeId} className="flex items-center justify-between px-3 py-2 bg-gray-50 rounded-lg">
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {availableEmps.map(e => <option key={e.employeeId} value={e.employeeId}>{e.fullName} ({e.positionName})</option>)}
                  </select>
