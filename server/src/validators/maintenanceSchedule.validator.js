...
function up(v) { return typeof v === 'string' ? v.toUpperCase() : v; }
...
export function createScheduleSchema(body) {
  ...
  if (!body.maintenanceType || !VALID_TYPES.includes(up(body.maintenanceType))) return `Loại bảo trì không hợp lệ. Hợp lệ: ${VALID_TYPES.join(', ')}`;
  ...
  if (body.frequencyUnit && !VALID_UNIT.includes(up(body.frequencyUnit)))    return `FrequencyUnit không hợp lệ. Hợp lệ: ${VALID_UNIT.join(', ')}`;
  if (body.priority && !VALID_PRIORITY.includes(up(body.priority)))          return `Priority không hợp lệ. Hợp lệ: ${VALID_PRIORITY.join(', ')}`;
  return null;
}

export function updateScheduleSchema(body) {
  if (body.startDate && isNaN(Date.parse(body.startDate)))                   return 'Ngày bắt đầu không hợp lệ';
  if (body.frequencyUnit && !VALID_UNIT.includes(up(body.frequencyUnit)))    return 'FrequencyUnit không hợp lệ';
  if (body.priority && !VALID_PRIORITY.includes(up(body.priority)))          return 'Priority không hợp lệ';
  return null;
}
