...
export async function findAll({ limit, offset, status, assetTypeId, locationId, search } = {}) {
  ...
  if (search)      {
    where += ' AND (a.AssetName LIKE ? OR a.SerialNumber LIKE ? OR a.Manufacturer LIKE ?)';
...
export async function count({ status, assetTypeId, locationId, search } = {}) {
  ...
  if (search) {
    where += ' AND (AssetName LIKE ? OR SerialNumber LIKE ? OR Manufacturer LIKE ?)';
    params.push(`%${search}%`, `%${search}%`, `%${search}%`);
  }
...
export async function create({ assetName, assetTypeId, locationId, status, commissionDate, manufacturer, serialNumber, photo, qrCodePath, description }) {
  const [result] = await getPool().query(
    `INSERT INTO Assets (AssetName, AssetTypeID, LocationID, Status, CommissionDate, Manufacturer, SerialNumber, Photo, QRCodePath, Description)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
...
export async function update(id, fields) {
  const map = {
    assetName: 'AssetName', assetTypeId: 'AssetTypeID', locationId: 'LocationID',
    status: 'Status', commissionDate: 'CommissionDate', manufacturer: 'Manufacturer',
    serialNumber: 'SerialNumber', photo: 'Photo', qrCodePath: 'QRCodePath', description: 'Description',
  };
  const setClauses = [];
  const params = [];
