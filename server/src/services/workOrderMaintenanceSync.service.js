...
export async function bumpStaleCalendarSchedules(assetId, completedDateStr) {
  ...
  for (const s of rows) {
    ...
    if (s.nextDueDate <= completedDateStr) {
      const next = calcNextDueDate(completedDateStr, s.frequencyValue, s.frequencyUnit);
...
export async function afterWorkOrderCompleted(wo) {
  ...
  const totalRt = counter != null ? Number(counter.totalAccumulatedHours) : null;
  ...
  await historyModel.create({
    ...
    actualHours:         wo.actualHours != null && wo.actualHours !== '' ? Number(wo.actualHours) : null,
  ...
  if (counter) {
    await assetCounterModel.setLastMaintenanceTotal(wo.assetId, counter.totalAccumulatedHours);
